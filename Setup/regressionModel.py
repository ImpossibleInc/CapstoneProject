...
from Setup.databaseManipulation import create_lms, drop_lms
from Setup.databaseManipulation import fill_lms
...
def setup_dataframes():
    ...
    return (df_array, i, strings)

def train_model():
    df_array, i, strings = setup_dataframes()
    drop_lms()
    create_lms()
...
def lm_stats():
    ...
    for df_item in df_array:
        ...
        if 1 <= i <= len(strings):
            string = strings[i - 1]
        print("\n" + string + " Analysis")
        X = df_item[['age', 'prev_games_played', 'prev_games_started', 'prev_passing_completions',
                     'prev_passing_attempts', \
                     'prev_passing_yards', 'prev_passing_TDs', 'prev_interceptions', 'prev_rushing_attempts',
                     'prev_rushing_yards', \
                     'prev_yards_per_attempt', 'prev_rushing_TDs', 'prev_targets', 'prev_receptions',
                     'prev_receiving_yards', 'prev_yards_per_reception', \
                     'prev_receiving_TDs', 'prev_fumbles', 'prev_fumbles_lost', 'prev_two_point_conversions',
